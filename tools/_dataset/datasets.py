...
class BaseDataset:
    ...
    def make_dataset(
        ...
        Float[Array, "dataset sequence"],
        Float[Array, "dataset sequence channels+1"],
        Tuple[Float[Array, "dataset sequence-1 channels+1"]],
        Float[Array, "dataset None"],
        int,
    ]:
        ...
        **Returns:**
        ...
        coeffs = tuple(
            [jnp.zeros((dataset_size, sequence_length - 1, channels + 1)) for _ in jnp.arange(4)]
        )
...
class SpiralDataset(BaseDataset):
    ...
    def make_dataset(
        ...
        **Returns:**
        ...
        ts = jnp.broadcast_to(
            jnp.linspace(0, 4 * math.pi, self.length), (dataset_size, self.length)
        )  # -> (256, 100)
...
class MNISTStrokeDataset(BaseDataset):
    ...
    def __init__(
        ...
        *,
        ...
        input_format: str = "point_sequence",
    ...
    def make_dataset(
        ...
        Sequence[Float[Array, "sequence"]],
        Sequence[Float[Array, "sequence channels+1"]],
        Tuple[Sequence[Float[Array, "sequence-1 channels+1"]]],
        Sequence[Float[Array, "sequence None"]],
        int,
    ]:
        """Creat or load records.
        ...
        **Returns:**
        ...
        if self.mode_train:
            if self.input_format == "point_sequence":
                path_list_input_sequence = glob.glob(
                    self.config.train_data_point_sequence, recursive=True
                )
            elif self.input_format == "input_sequence":
                path_list_input_sequence = glob.glob(
                    self.config.train_data_input_sequence, recursive=True
                )
            path_label = self.config.train_label
        else:
            if self.input_format == "point_sequence":
                path_list_input_sequence = glob.glob(
                    self.config.test_data_point_sequence, recursive=True
                )
            elif self.input_format == "input_sequence":
                path_list_input_sequence = glob.glob(
                    self.config.test_data_input_sequence, recursive=True
                )
        ...
        coeffs = [[], [], [], []]
        dim_ys = ys[0].shape[-1]
        for _ts, _ys in zip(ts, ys, strict=False):
            _coeffs = func_interpolation(
                _ts, _ys, fill_forward_nans_at_end=True, replace_nans_at_start=jnp.zeros(dim_ys)
            )
    ...
    def _make_timestamp(
        self, points: Tuple[Float[Array, "sequence"], Float[Array, "sequence"]]
    ) -> Float[Array, "sequence"]:
        ...
        **Returns:**
        ...
        for point_curr in zip(*points, strict=False):
    ...
    def _random_mask_nan(
        self, input_sequence: Float[Array, "sequence channels"], key: PRNGKeyArray, p: float = 0.0
    ) -> Float[Array, "sequence channels"]:
