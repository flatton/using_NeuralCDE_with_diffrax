...
def test_module(seed: int = 5678, steps: int = 2, batch_size: int = 2) -> None:
    ...
    dataset_size = 4
    sequence_length = 6
    timestamps = jnp.broadcast_to(jnp.linspace(0, 4 * math.pi, sequence_length), (dataset_size, sequence_length))
    ...
    for step_i, data_i in zip(range(steps), dataloader.dataloader((series_data, labels), batch_size, key=loader_key)):
