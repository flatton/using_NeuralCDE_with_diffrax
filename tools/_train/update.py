...
class Trainer:
    ...
    def make_step(
        self,
        model: eqx.Module,
        data: Tuple[Float[Array, "batchsize"], ...],
        opt_state: OptimizerState,
        *args,
        key: PRNGKeyArray,
    ) -> Tuple[Float, Float, eqx.Module, OptimizerState]:
...
def run_train(
    ...
    *args,
    ...
    for step, data in zip(
        range(steps),
        dataloader_ununiformed_sequence(((ts, *coeffs), labels), batch_size, key=loader_key), strict=False,
    ):
        train_step_key, _train_step_key = jr.split(train_step_key, 2)
        start = time.time()
        xe, acc, model, opt_state = trainer.make_step(
            model, data, opt_state, *args, key=_train_step_key
        )
        end = time.time()
