...
class ExperimentConfig(BaseModel):
    ...
    def validate_input_format(cls, input_format: str) -> str:
        ...
        if input_format not in options:
            raise ValueError(
                f"Please select one of the following options: {options}. But now {input_format}"
            )
        return input_format
    ...
    def validate_interpolation(cls, interpolation: str) -> str:
        ...
        if interpolation not in options:
            raise ValueError(
                f"Please select one of the following options: {options}. But now {interpolation}"
            )
    ...
    def validate_input_format(cls, neural_model_name: str) -> str:
        ...
        if neural_model_name not in options:
            raise ValueError(
                f"Please select one of the following options: {options}. But now {neural_model_name}"
            )
